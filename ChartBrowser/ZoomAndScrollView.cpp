	...
	if (pCollection)
	{
		...
		if (pSeries1)
		{
			...
			if (pPoints)
			{
				...
				pPoints->AddPoints(nBatchCount);

				// Fill series data
				double yValue = 50.0;
				for(int pointIndex = 0; pointIndex < nBatchCount; pointIndex ++)
				{
					yValue = yValue + ( (double)rand() / RAND_MAX * 10.0 - 5.0 );

					CXTPChartSeriesPoint* pPoint = pPoints->GetAt(pointIndex);

					pPoint->SetArgumentValue(pointIndex);
					pPoint->SetValue(0, yValue);
				}
#else
