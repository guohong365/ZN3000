...
void CDateTimeView::AddTimeSeries()
{
	CXTPChartContent* pContent = m_wndChartControl.GetContent();
	
	pContent->GetTitles()->GetAt(0)->SetText(_T("Timestamps"));
	
	CXTPChartSeries* pSeries = pContent->GetSeries()->GetAt(0);
	
	pSeries->GetPoints()->RemoveAll();

	CXTPChartTime time(10,0,0,0);

	for (int nPoint=0; nPoint<100; nPoint++)
	{
		pSeries->GetPoints()->Add(new CXTPChartSeriesPoint(time.m_dTime, (rand() % 100) / 24.0));

		time.m_dTime += (rand() % 1000);
	}
	...
	pDiagram->GetAxisX()->SetDateTimeScaleUnit(xtpChartScaleMinute);
...
void CDateTimeView::AddYearSeries()
{
	...
	pSeries->GetPoints()->Add(new CXTPChartSeriesPoint(COleDateTime(2012, 1, 1, 0, 0, 0), (rand() % 100)  * 30.0 * 12));
