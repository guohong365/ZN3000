...
#include "StepAreaView.h"


// CStepAreaView

IMPLEMENT_DYNCREATE(CStepAreaView, CBaseView)

CStepAreaView::CStepAreaView()
	: CBaseView(CStepAreaView::IDD)
{
...
void CStepAreaView::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CStepAreaView, CBaseView)
...
// CStepAreaView diagnostics

#ifdef _DEBUG
void CStepAreaView::AssertValid() const
{
	CBaseView::AssertValid();
}

#ifndef _WIN32_WCE
void CStepAreaView::Dump(CDumpContext& dc) const
{
	CBaseView::Dump(dc);
}
#endif
...
// CStepAreaView message handlers


void CStepAreaView::OnInitialUpdate() 
{
...
void CStepAreaView::SetAxisTitle(CXTPChartDiagram2D* pDiagram, CString strAxisX, CString strAxisY)
{
	if (pDiagram)
	{
...
void CStepAreaView::CreateChart()
{
	...
	if (pCollection)
	{
		...
		if (pSeries1)
		{
			...
			if (pPoints)
			{
				...
				pPoint = pPoints->Add(new CXTPChartSeriesPoint(_T("June, 2005"), 136.6));
			}

			CXTPChartStepAreaSeriesStyle* pStyle = (CXTPChartStepAreaSeriesStyle*)
				pSeries1->SetStyle(new CXTPChartStepAreaSeriesStyle());
...
void CStepAreaView::OnCheckShowLabels() 
{
	UpdateData();
...
void CStepAreaView::OnCheckShowMarkers() 
{
	UpdateData();
...
void CStepAreaView::OnCbnSelchangeComboMarkerSize()
{
	UpdateData();
...
void CStepAreaView::OnCbnSelchangeComboMarkerType()
{
	UpdateData();
...
void CStepAreaView::OnCheckInvertedStep()
{
	UpdateData();
	CXTPChartSeriesCollection* pSeriesCollection = m_wndChartControl.GetContent()->GetSeries();
	...
	for (int i = 0; i < pSeriesCollection->GetCount(); i++)
	{
		CXTPChartStepAreaSeriesStyle* pStyle = (CXTPChartStepAreaSeriesStyle*)pSeriesCollection->GetAt(i)->GetStyle();
