	...
	float scale, float zero)
{
	...
	float current=start;
	int x0 = startX;
	int y0 =zero - int(buffer.buffer()[start%bufferSize] * scale);
	for(int i=startX + 1; i<=endX; ++i)
	{
		current +=step;
	    const int x1=i;
	    const int y1=zero - buffer.buffer()[int(current)%bufferSize] * scale;
	...
	float scale, float zero)
{
	...
	for(int i=start + 1; i<=end; ++i)
	{
	    ...
	    const int y1=zero - buffer.buffer()[i%bufferSize] * scale;
		graphics.DrawLine(pen, x0, y0, x1, y1);
		x0=x1;
		y0=y1;
	}
...
void WaveDrawer::OnDraw( Gdiplus::Graphics & graph )
{
	...
	SignalBuffer<double> & buffer=_pSignal->signalBuffer();
	const size_t current=buffer.length();
    const size_t totalSampleCount=int(
		GetSize().Width / ScreenInfo::GetScreenInfo().GetDpmmX() / GetVelocity() * _pSignal->
		samplingFrequency());
	...
	info.Format(_T("total:%.2fs, start: %ld, length: %d. "), current / getChannelBuffer()->samplingFrequency(),
...
void WaveDrawer::OnSizeChanged()
{
	...
	{
		if(_pSignal->signalBuffer().minimum()>0)
		{
			_baseline=GetSize().Height* 9/10;
		}
		else
		{
			_baseline = GetSize().Height*(1 - 8*(_pSignal->signalBuffer().minimum()/(_pSignal->signalBuffer().maximum() - _pSignal->signalBuffer().minimum())))/10;
		}
	}
	_waveHeight=GetSize().Height*_scale;
...
void WaveDrawer::_drawErase(Gdiplus::Graphics& graphics, SignalBuffer<double>& buffer, size_t totalSampleCount, double sampleDotSpacing)
{
	...
	graphics.FillEllipse(&spot, _lastEndX + drawWidth, _baseline - int(buffer.buffer()[current % buffer.size()] * _scale * height / 32767), spotSize, spotSize);
