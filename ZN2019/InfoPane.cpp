...
void CInfoPane::RefreshInfo()
{
	if (_pPatient)
	{
		_id.Format(_T("编号：%s"), _pPatient->getId());
		_name.Format(_T("姓名：%s"), _pPatient->getName());
		_gender.Format(_T("性别：%s"), _pPatient->getGender() == 0 ? _T("男") : _T("女"));
		_height.Format(_T("身高：%3.1f (cm)"), _pPatient->getHeight());
		_weight.Format(_T("体重：%3.1f (Kg)"), _pPatient->getWeight());
		_bloodPressure.Format(_T("血压：%3.1f / %3.1f (mmHg)"), _pPatient->getDiastolicBloodPressure(),
		                      _pPatient->getSystolicBloodPressure());
		if (_pPatient->getHeartRate() != 0)
		{
			_heartRate.Format(_T("心率：%d"), int(_pPatient->getHeartRate()));
		}
		else
		{
			_heartRate = _T("心率：");
