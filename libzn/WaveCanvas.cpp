...
void WaveCanvas::PrepareCanvas( int dx, int dy )
{
	...
	//graph.ScaleTransform(ScreenInfo::GetScreenInfo().GetDpmmX()/10, ScreenInfo::GetScreenInfo().GetDpmmY()/10);
	_pWaveBackground->Draw(graph);
	for (int i=0; i< _waveDrawers.size(); i ++)
	{
...
void WaveCanvas::DrawHorizLabel(Gdiplus::Graphics & graph)
{
	Gdiplus::SolidBrush brush(Gdiplus::Color::LightGreen);
	Gdiplus::RectF layout(0, _pWaveBackground->GetPosition().Y + _pWaveBackground->GetSize().Height + 10, 
		1000,(GetSize().Height - _pWaveBackground->GetSize().Height/2));

	Gdiplus::StringFormat stringFormat;
	stringFormat.SetLineAlignment(Gdiplus::StringAlignmentNear);
	stringFormat.SetAlignment(Gdiplus::StringAlignmentCenter);
	float hInc= (float)500.0f/_waveDrawers[0]->GetVelocity();
	int hStep= _pWaveBackground->GetSize().Width / 500;
	float t;
	CString strLebel;
