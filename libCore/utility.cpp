...
namespace uc
{
	namespace utils
	{
		TCHAR* utils::UpperString(TCHAR* str)
		{
			TCHAR* p = str;
			while (*p)
			{
	...
	std::string retVal=StringDigitalToChCharA(text);
		...
		*/
		...
		void ReorderBytes(unsigned char* outBuffer, const unsigned char* inBuffer, unsigned char* mapBuffer,
		                  size_t count)
		{
			for (size_t i = 0; i < count; i++)
			{
		...
		void RestoreBytesOrder(unsigned char* outBuffer, const unsigned char* inBuffer, unsigned char* mapBuffer,
		                       size_t count)
		{
			for (size_t i = 0; i < count; i++)
			{
		...
		std::wstring mbstows(const std::string& str, const char* locale)
		{
			...
			const size_t size = 1 + mbstowcs(nullptr, str.c_str(), 0);
		...
		std::string wstombs(const std::wstring& wstr, const char* locale)
		{
			const std::locale old;
			std::locale::global(std::locale(locale));
			const size_t size = wcstombs(nullptr, wstr.c_str(), 0) + 1;
